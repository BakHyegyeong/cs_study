...
- 데이터 정확성 : 중복이나 누락이 없는 값이 올바른 상태
- 데이터 일관성 : 조회했을 때 항상 같은 결과가 나오는 것

### 무결성과 정합성

- 무결성 : **데이터의 정확성, 일관성을 유지**하는 것
    
    → 데이터에 결함이 없는 상태
    
- 정합성 : 데이터들의 값이 서로 **일치하는 것**

어떤 데이터는 정합성은 이상이 없으나, 무결성이 훼손될 수 있다. (중복 데이터가 다 틀린 값으로 일치 - 정합성 O, 무결성 X)
무결성이 정합성보다 더 큰 범위로 데이터베이스에서 가장 큰 목표는 데이터 무결성을 높이는 것이다.
...
사용자가 요청할 때마다 MySQL에서 connection을 맺는 것이 아닌
**Connection pool에 있는 미리 생성되어있는 connection을 가져와 사용**하는 것
→ 통신 속도 성능이 향상

![](https://img1.daumcdn.net/thumb/R1280x0/?scode=mtistory2&fname=https%3A%2F%2Fblog.kakaocdn.net%2Fdn%2FcnDaSa%2Fbtq9PTdID0v%2FWlIEVwtkvQn45CLUsjwKM1%2Fimg.png)
...
데이터베이스에서 동시성을 제어하는 방법 중 하나로, **데이터의 버전(snapshot)을 관리**함으로써 동시성을 관리

→ Lock을 사용하지 않고 일관된 읽기를 제공하는 것

### 동작 원리

**원본의 데이터와 변경중인 데이터를 동시에 유지**하는 방식으로 이루어진다.

- 원본 데이터에 대한 Snapshot을 백업하여 보관
- 새로운 사용자가 데이터에 접근하면 **데이터베이스의 Snapshot을 생성한다.**
- 변경이 취소되면 원본 Snapshot을 바탕으로 데이터를 복구하고, 만약 변경이 완료되면 최종적으로 디스크에 반영하는 방식으로 동작

**예시**

1. 트랜잭션 A가 이름이 T-shirt인 상품을 조회하면, T-shirt의 **현재 버전(읽기 시점의 데이터 상태)을 저장**한다.

2. 동시에 트랜잭션 B가 T-shirt를 Shoes으로 수정할 경우, **새로운 Shoes라는 이름의 상품 버전(스냅샷)을 생성**한다.

3. 트랜잭션 A는 자신이 조회하기 시작한 시점의 데이터(T-shirt)인 버전을 계속 읽게 되며, 수정 중인 데이터와는 격리된다.

### 특징

- 일반적인 RDBMS보다 매우 빠르게 작동할 수 있다.
- 사용하지 않는 데이터가 계속 쌓이게 되므로 데이터를 정리하는 시스템이 필요하다.
- 데이터 버전이 충돌하면 애플리케이션 영역에서 이러한 문제를 해결해야 한다.

+) [MVCC와 LOCK 제어](https://mangkyu.tistory.com/53)
...
| 많은 기능을 지원하기에 상대적으로 고사양의 서버를 요구한다. |  가벼우며 요구하는 서버의 사양이 낮다. |